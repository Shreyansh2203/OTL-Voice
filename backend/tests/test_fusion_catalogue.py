...
def mock_db(tmp_path):
    ...
    with (
        patch("backend.services.fusion_catalogue._get_db", side_effect=_mock_get_db),
...
def test_fetch_all_projects():
    ...
    resp2.json.return_value = {
        "items": [{"ProjectId": f"P{i + 100}"} for i in range(50)]
    }
...
@patch("backend.services.otl_client.service_credential")
def test_do_load_catalogue(
    mock_service_credential,
    mock_assignments,
    mock_members,
    mock_tasks,
    mock_projects,
    ...
    mock_tasks.return_value = [
        {"TaskId": "T1", "TaskNumber": "1", "TaskName": "Task 1"}
    ]
                ...
                [
                    {
                        ...
                        "project_name": "A",
                        "tasks": [
                            {"task_number": "1", "task_id": "T1", "task_name": "T"},
                            {"task_number": None, "task_id": None, "task_name": "T3"},
                        ],
                    }
                ]
            ),
