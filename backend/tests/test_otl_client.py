...
def test_map_entry_to_otl():
    ...
    with patch("backend.services.otl_client.datetime") as mock_dt:
        mock_dt.now.return_value.replace.return_value = None # not used here since date provided
    ...
    assert "T09:00" in ev3["startTime"] # default start time
