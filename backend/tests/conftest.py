...
def mock_otl_client():
    with (
        ...
        patch("backend.api.v1.auth.otl_client", mock),
        patch("backend.api.v1.chat.otl_client", mock),
        patch("backend.api.v1.timecards.otl_client", mock),
...
def mock_fusion_catalogue():
    with (
        patch("backend.main.fusion_catalogue") as mock,
        patch("backend.api.v1.chat.fusion_catalogue", mock),
        patch("backend.api.v1.timecards.fusion_catalogue", mock),
        patch("backend.api.v1.health.fusion_catalogue", mock),
    ):
        mock.alist_assignments_for_worker = AsyncMock(return_value=[])
...
def mock_speech_client():
    with (
        patch("backend.main._speech_client") as mock,
        patch("backend.api.v1.chat._speech_client", mock),
    ):
