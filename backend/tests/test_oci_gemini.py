...
def test_service_endpoint():
    assert (
        _service_endpoint("us-ashburn-1")
...
@patch.dict(
    os.environ,
    {
        "OCI_COMPARTMENT_ID": "c1",
        "OCI_REGION": "r1",
        "OCI_USER_OCID": "ocid1.user.oc1..aaaaaa",
        "OCI_TENANCY_OCID": "ocid1.tenancy.oc1..bbbbb",
        "OCI_FINGERPRINT": "00:11:22:33:44",
        "OCI_PRIVATE_KEY": "k1",
    },
    clear=True,
)
...
def test_complete_fallback_json(mock_client):
    ...
    mock_resp.data.__str__.return_value = json.dumps(
        {"text": "fallback text", "nested": [{"text": " 2"}]}
    )
    client.client.chat.return_value = mock_resp
...
def test_stream_empty_events(mock_client):
    ...
    mock_resp.data.events.return_value = [
        MagicMock(data=""),
        MagicMock(data=json.dumps({"other": "no text"})),
    ]
    client.client.chat.return_value = mock_resp
