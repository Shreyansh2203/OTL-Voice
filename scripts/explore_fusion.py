...
if not DEFAULT_OTL_URL:
    print(
        "Error: OTL_BASE_URL environment variable is not set. Please configure it in .env"
    )
    sys.exit(1)
...
def main():
    ...
    print("=" * 65)
    use_default = (
        input(f"Use configured credentials ({DEFAULT_USERNAME})? [Y/n]: ")
        .strip()
        .lower()
    )
    ...
    while True:
        ...
        if choice == "0":
            ...
            search_val = input(
                "Enter Person Number (or press Enter to list first 10): "
            ).strip()
            if search_val:
                url = f"{HOST_URL}/hcmRestApi/resources/11.13.18.05/workers"
                params = {"q": f"PersonNumber='{search_val}'", "expand": "names,emails"}
            else:
                url = f"{HOST_URL}/hcmRestApi/resources/11.13.18.05/workers"
            ...
            if resp.status_code == 200:
                ...
                for p in projects:
                    print(
                        f"   * Project #{p.get('ProjectNumber')} | '{p.get('ProjectName')}' | Status: {p.get('ProjectStatus')} | Mgr: {p.get('ProjectManagerName')} | ID: {p.get('ProjectId')}"
                    )
            else:
                print(f"-> Error ({resp.status_code}): {resp.text[:300]}")
        elif choice == "3":
            proj_id = input(
                "Enter Project ID or Project Number (e.g., 300000040441155 or 101110): "
            ).strip()
            if not proj_id:
            ...
            if task_resp.status_code == 200:
                ...
                for t in tasks:
                    print(
                        f"   * Task #{t.get('TaskNumber')}: '{t.get('TaskName')}' (ID: {t.get('TaskId')})"
                    )
            else:
                print(
                    f"-> Tasks error ({task_resp.status_code}): {task_resp.text[:200]}"
                )
            tm_resp = client.get(
                f"{HOST_URL}/fscmRestApi/resources/11.13.18.05/projects/{proj_id}/child/ProjectTeamMembers"
            )
            if tm_resp.status_code == 200:
                ...
                for m in members:
                    print(
                        f"   * {m.get('PersonName')} (Role: {m.get('ProjectRole')}, Email: {m.get('Email')})"
                    )
            else:
                print(
                    f"-> Team Members error ({tm_resp.status_code}): {tm_resp.text[:200]}"
                )
        elif choice == "4":
            emp_no = input(
                "Enter Person Number (optional, leave blank for all): "
            ).strip()
            params = {"limit": 10}
            if emp_no:
            ...
            if resp.status_code == 200:
                ...
                for item in items:
                    print(
                        f"   * Request ID: {item.get('timeRecordEventRequestId')} | Mode: {item.get('processMode')} | Inline: {item.get('processInline')}"
                    )
            else:
                print(f"-> Error ({resp.status_code}): {resp.text[:300]}")
        elif choice == "5":
            endpoint = input(
                "Enter endpoint path (e.g. /hcmRestApi/resources/11.13.18.05/workers): "
            ).strip()
