...
def run_suite():
    ...
    if resp.status_code == 200:
        ...
        for w in items:
            ...
            print(
                f"   * Person #{w.get('PersonNumber')}: {name} (PersonId: {w.get('PersonId')})"
            )
    else:
    ...
    proj_resp = client.get(
        ...
        params={
            "fields": "ProjectId,ProjectNumber,ProjectName,ProjectManagerName",
            "limit": 3,
        },
    )
    if proj_resp.status_code == 200:
        ...
        for p in projects:
            ...
            task_resp = client.get(
                f"{HOST_URL}/fscmRestApi/resources/11.13.18.05/projects/{p_id}/child/Tasks",
                params={"limit": 4},
            )
            ...
            tm_resp = client.get(
                f"{HOST_URL}/fscmRestApi/resources/11.13.18.05/projects/{p_id}/child/ProjectTeamMembers"
            )
            if tm_resp.status_code == 200:
                ...
                if members:
                    ...
                    for m in members:
                        print(
                            f"       - {m.get('PersonName')} ({m.get('ProjectRole')})"
                        )
    else:
    ...
    if otl_resp.status_code == 200:
        ...
        for item in tc:
            print(
                f"   * Request ID: {item.get('timeRecordEventRequestId')}, Mode: {item.get('processMode')}"
            )
    else:
