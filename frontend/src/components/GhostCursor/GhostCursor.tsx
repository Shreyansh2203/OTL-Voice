...
function createFilmGrainShader(grainIntensity: number) {
  return {
    uniforms: {
      tDiffuse: { value: null },
      iTime: { value: 0 },
...
const GhostCursor: React.FC<GhostCursorProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      filmPass = new ShaderPass(createFilmGrainShader(grainIntensity) as any);
      filmPassRef.current = filmPass;
      composer.addPass(filmPass);

      composer.addPass(new ShaderPass(UnpremultiplyShaderDef));
    } catch {
      renderer.dispose();
    ...
    const ensureLoop = () => {
      if (!runningRef.current && !document.hidden && !prefersReducedMotion()) {
  ...
  useEffect(() => {
    ...
    if (!canvas) return;
    if (mixBlendMode) {
      canvas.style.setProperty('mix-blend-mode', String(mixBlendMode));
    } else {
